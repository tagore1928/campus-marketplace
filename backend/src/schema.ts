 ...
 * This file outlines the structural definitions and types for all Firestore collections.
 */
...
// Social Posts Collection (social_posts in db)
export interface SocialPostDocument {
  id: string;            // Document ID
  content: string;       // Text payload
  images: string[];      // Optional compressed Cloudinary image URLs (max 2)
  creatorId: string;     // Author user UID
  creatorName: string;   // Author name
  creatorEmail: string;  // Author email
  college: string;       // Visibility college name, or "Global"
  ...
  reportsCount: number;  // Moderation flag counter
  reportedBy: string[];  // User UIDs that reported this post
  createdAt: string;     // ISO timestamp
  updatedAt?: string;    // ISO timestamp when edited
}
